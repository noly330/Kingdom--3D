...
public class CombatListSO : ScriptableObject
{
    ...
    {
        if(index > _combatList.Length - 1)  return null;
        if(vfxIndex > _combatList[index].sfxConfigs.Length - 1)  return null;
        return _combatList[index].sfxConfigs[vfxIndex];
    }

    public CombatSFXConfig TryGetSFXConfig(int index, int vfxIndex)
    {
        if(index > _combatList.Length - 1)  return null;
